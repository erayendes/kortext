...
- [ ] **"Proje hazırlanıyor" ekranı — etkileşim bug'ları** (2026-06-05, DECISIONS Bölüm 14.5). Ekran (`src/routes/initializing.tsx`) doğru veriyi gösteriyor ama: (1) Sidebar "Setup" linki hash-router-farkında değil → `/initializing#/initializing` üretiyor, doğrudan gidince Dashboard'a düşüyor; (2) **satırdaki Onayla `stopPropagation` eksik** → tıklama satıra yayılıp drawer açıyor, onaylamıyor (ekrandan onay şu an çalışmıyor; Dashboard "For review" kartından çalışıyor); (3) drawer içi onay/revize aksiyonları beklenen yerde değil; (4) <500px responsive overlap. Düşük-orta efor, hepsi `src/` içinde.
- [ ] **Kapı Faz 2 — "revize tek başına döner"** (2026-06-05, DECISIONS Bölüm 14.2). Analiz kapıları DAG-paralel (✅ canlı kanıt: LEGAL ∥ GROWTH aynı anda onayda). Ama `reject` hâlâ **tüm run'ı abort** ediyor. Eray'ın seçtiği semantik (option 1): bir kapı reddedilince **sadece o adım** yeniden üretilsin, kardeşlerin onayı dursun, run ölmesin. `worker-pool` reject yolu + `Orchestrator.retryRun` (abort-on-reject) ayıklanmalı. Mevcut `tests/worker-pool-gate.test.ts` reject→cancel + retryRun davranışını sabitliyor — semantik değişince güncellenecek.
- [ ] **Canlı koşu — epic/versiyon/model + dev-cycle paralelliği** (2026-06-05, DECISIONS Bölüm 14.1/14.3). Ingestion (epic/version/parent/model sütunları) + driver Phase 2/3 paralelliği **744 test yeşil** ama **gerçek-Claude ile canlı kanıt yapılmadı**. Mock ile analiz kapıları kanıtlandı. ~25dk, executor=claude, UAT kum havuzu.
