...
- [ ] **⚠️ Canlı koşu BULGUSU — backlog enrichment + step-8 raporu** (2026-06-05, yukarıdaki koşudan). Asıl §14.2 kanıtlandı ama zincir iki gerçek sorun gösterdi: **(A) epic/version/model Board'a inmiyor.** `backlog.yaml` yalnız step-0'da (bir kez, 19:21) yazıldı; enrichment adımları (4-7: "Epic ve Versiyon", "Atama: model") backlog.yaml'i **yeniden yazmadı** → version/model hiç üretilmedi (yaml'da 0 satır). Ayrıca **alan-adı uyuşmazlığı:** persona `epic: "Infrastructure"` (etiket string) yazıyor ama ingester `parent_epic:`/`parent:` bekliyor (yaml'da `epic:`×102, `parent_epic:`×0) + hiç `type: epic` item üretilmedi (düz 100 task) → `parent_epic`/`version`/`model` sütunları **0**. **(B) step-8 (konsolidasyon, +operation-manager) FAILED:** "declared outputs not produced: `.kortext/reports/planning-reports_<slug>_<ts>.md`" — oysa dosya 19:59'da yazıldı (`planning-reports_planning_20260605.md`). output-resolver pattern/timing sorunu; run, backlog ingest'ten SONRA failed işaretlendi. Çözüm yönü: planning-pipeline persona talimatlarını `parent_epic`/`version`/`model` anahtarlarını + gerçek `type: epic` item'ları üretecek + son backlog.yaml'i yeniden yazacak şekilde kalibre et (alttaki "Backlog köprüsü — zenginleştirme" ile birleşir); step-8 rapor output pattern'ini incele. Kanıt: `kortext-live-uat/.kortext/`.
  - **Dokunulacak dosyalar (temiz oturum giriş noktası):** `workflows/planning-pipeline.md` (step 4-5 epic/versiyon + step 6-7 model atama talimatları — yaml anahtarlarını `parent_epic:`/`version:`/`model:` yap, `type: epic` container item'ları zorunlu kıl, son adımda backlog.yaml'i bütün halinde yeniden yaz) · `server/engine/backlog-ingest.ts:205-216` (alan eşleme — istenirse `epic:` alias'ını da `parent_epic`'e kabul et) · step-8 için `server/engine/output-resolver.ts` (`<slug>`/`<ts>` pattern eşleşmesi neden `planning-reports_planning_20260605.md`'yi yakalamadı) + `workflows/planning-pipeline.md` son adım output bildirimi.
  - **Yeniden test:** sandbox `kortext-live-uat` diskte duruyor (`backlog.yaml` + 100 item). Sıfırdan koşmak için: `cd kortext-live-uat && rm -rf .kortext && git init` → `KORTEXT_PORT=3300 npx tsx <repo>/server/index.ts &` → `POST /api/blueprint` (executor=claude, projectDir=null, TaskFlow BRD) → /tmp/kx-autoapprove*.sh deseni ile kapıları onayla → `/api/backlog`'ta `parent_epic`/`version`/`model` sayılarını kontrol et. Tam runbook bu oturumun handover'ında.
