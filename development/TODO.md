...
- [ ] **Tek-seferlik kesintisiz canlı koşu** — sıfırdan onboarding → analiz → planning → Board (executor=claude, UAT kum havuzu). **2026-06-05 ilerleme:** (1) ilk deneme analysis adım 9'da geçici socket hatasıyla düştü → §14.8 transient retry eklendi; (2) ikinci deneme analysis'i tamamladı (30 dk, adım 9 retry'la kurtuldu — canlı kanıt), planning'e geçti, **epic/parent_id/gates canlı indi** (§14.7 çekirdeği kanıtlandı) ama enrichment ~22dk/adım olduğu için kesildi → §14.9 delta köprüsü eklendi. **Kalan:** §14.9 ile yeniden koş, planning'in artık dakikalar sürdüğünü + 5/5 sütunun (epic/parent/version/model/gate) dolduğunu canlı doğrula.
