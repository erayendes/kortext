...
- [ ] **Tek-seferlik kesintisiz canlı koşu** — sıfırdan onboarding → analiz → planning → Board (~25dk, executor=claude, UAT kum havuzu). Her halka ayrı kanıtlandı; kesintisiz tam zincir koşulmadı. **2026-06-05 denemesi:** sandbox `kortext-live-uat-v2`'de analysis adım 0-8 gerçek Claude ile başarılı, **adım 9 (+engineering-manager) geçici `claude` CLI hatasıyla düştü** ("API Error: The socket connection was closed unexpectedly", exit 1) → planning'e ulaşmadı, §14.7 canlı teyit edilemedi (ama 6 unit test + in-memory e2e ile kanıtlı). Tekrar dene (geçici hata, kod değil).
- [ ] **⚠️ Dayanıklılık — adım-seviyesi transient retry** (2026-06-05 canlı koşu bulgusu). Headless `claude` CLI uzun deep-research adımlarında **geçici API/socket hataları** alabiliyor (exit 1, `aborted=false`); şu an tek bir blip tüm run'ı `failed` yapıyor (`retryRun` yalnız startup `orphaned:` crash-recovery için, transient CLI exit'i kapsamıyor). Çözüm: CLI-executor'da transient sınıflandırma (socket closed / API Error / rate-limit) → exponential backoff ile N-kez step-level retry; kalıcı hatalardan (declared-output missing, validation) ayır. Üretimde canlı ajanlar bu hataları düzenli alacak → kesintisiz zincir için şart.
