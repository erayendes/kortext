...
- **Bağımlılık sırası (doğrulandı, Layer 0→3):** L0 paralel [**1✅**, **2✅**, **3✅**, 11(bağımsız)] → L1 [**4✅**, **uat✅**, 5(←1+2), 7] → L2 [6(←1+4), 9(←1)] → L3 [8(←6), 10 capstone]. **Layer 0 + Madde 4 + uat review-cycle TAMAM → test→review→done/bounce yaşam döngüsü uçtan uca çalışıyor (mock-first). Sıradaki adaylar: 6 (mekanik kapanış, ←artık açık; onay→done arası merge/handover/worktree) · 5 (whose-turn türetimi) · 9 (block→cancel, AbortController registry gerekir).**
