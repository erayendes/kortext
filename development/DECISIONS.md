...
**DAG:** step 1 `backlog.yaml`, adım 2-9 `backlog.patch.yaml` output verir; hiçbiri input almaz → döngüsüz lineer 1→9 (`buildGraph` doğrular). **Uçtan uca (in-memory):** full skeleton + 4 ardışık patch (gate×2/version/model) → tüm alanlar birleşti, clobber yok, review_gates 3'lü union. **767 test yeşil.** Beklenen kazanç: enrichment adımı başına ~22 dk → saniye/dakika (ajan birkaç satır yazıyor).
