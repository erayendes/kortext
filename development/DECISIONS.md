...
- **Bağımlılık sırası (doğrulandı, Layer 0→3):** L0 paralel [**1✅**, **2✅**, **3✅**, 11(bağımsız)] → L1 [**4✅**, **uat✅**, **5✅**, 7] → L2 [6(←1+4), 9(←1)] → L3 [8(←6), 10 capstone]. **Layer 0 + Madde 4 + uat + Madde 5 TAMAM → test→review→done/bounce yaşam döngüsü uçtan uca çalışıyor (mock-first) + "sıra kimde" türetimi hazır (board tüketimi ayrı, src/). Sıradaki adaylar: 6 (mekanik kapanış, ←açık; onay→done arası merge/handover/worktree) · 9 (block→cancel, AbortController registry gerekir) · 7 (local test URL, PR-open).**
