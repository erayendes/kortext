...
- **Bağımlılık sırası (doğrulandı, Layer 0→3):** L0 paralel [**1✅**, **2✅**, **3✅**, 11(bağımsız)] → L1 [**4✅**, **uat✅**, **5✅**, 7] → L2 [**6✅**(←1+4), **9✅**(←1)] → L3 [**8✅**(←6), 10 capstone, 11 docs]. **Layer 0 + 4 + uat + 5 + 6 + 8 + 9 TAMAM → tek-item yaşam döngüsü (test→review→merge→done/bounce) + epic→staging tespit/tetik + block→cancel mekanizması çalışıyor (DÖRT mock-first arayüz + RunRegistry) + "sıra kimde" türetimi hazır. KALAN: 7 (local test URL, PR-open) · 10 (capstone: per-item run/worktree + dört mock'u gerçeğe bağla + worker-pool→registry wiring + tüm dikişleri kur) · 11 (bağımsız docs). Tek-item motoru ESASEN TAMAM; 10 = entegrasyon/gerçeğe-bağlama capstone'u.**
