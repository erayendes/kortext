...
- **Bağımlılık sırası (doğrulandı, Layer 0→3):** L0 paralel [**1✅**, **2✅**, **3✅**, 11(bağımsız)] → L1 [**4✅**, **uat✅**, **5✅**, 7] → L2 [**6✅**(←1+4), 9(←1)] → L3 [8(←6), 10 capstone]. **Layer 0 + Madde 4 + uat + 5 + 6 TAMAM → test→review→(merge)→done/bounce yaşam döngüsü uçtan uca çalışıyor (mock-first: gate exec / uat approver / merger üçü de mock) + "sıra kimde" türetimi hazır. Sıradaki adaylar: 9 (block→cancel, AbortController registry gerekir) · 7 (local test URL, PR-open) · 8 (epic→staging, ←6) · 10 (capstone: per-item run/worktree — mock'ların gerçek impl'lerini bağlar).**
