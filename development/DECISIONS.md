...
- **Bağımlılık sırası (doğrulandı, Layer 0→3):** L0 paralel [**1✅**, **2✅**, **3✅**, 11(bağımsız)] → L1 [**4✅**, **uat✅**, **5✅**, 7] → L2 [**6✅**(←1+4), 9(←1)] → L3 [**8✅**(←6), 10 capstone, 11 docs]. **Layer 0 + 4 + uat + 5 + 6 + 8 TAMAM → tek-item yaşam döngüsü (test→review→merge→done/bounce) + epic→staging tespit/tetik uçtan uca çalışıyor (DÖRT mock-first arayüz: gate exec / uat approver / merger / deployer) + "sıra kimde" türetimi hazır. Sıradaki adaylar: 9 (block→cancel, AbortController registry gerekir) · 7 (local test URL, PR-open) · 10 (capstone: per-item run/worktree + dört mock'u gerçeğe bağla + tüm dikişleri kur) · 11 (bağımsız docs).**
