...
**Seçenek A — Global kurulum (tavsiye edilen):**
```bash
npm install -g kortext
```
Kurulumdan sonra `kortext` komutu her yerden çalışır.

**Seçenek B — Yükleme yapmadan tek seferlik:**
```bash
npx kortext init
```
`npx` ile kalıcı kurulum gerekmez; npm otomatik indirir ve çalıştırır.

### 2.2 Projeye Kur

Proje klasörünün içindeyken tek komut:

```bash
kortext init
```

Bu komut otomatik olarak şunu yapar:
- `.kortext/` dizinini oluşturur ve tüm framework dosyalarını içine kopyalar
- `AGENTS.md`'yi proje köküne yerleştirir
- Git hook'larını kurar (`pre-commit`, `commit-msg`, `pre-push`)
- `.kortext/` ve `AGENTS.md`'yi git'e ekler (`git add`)

```
proje/
├── AGENTS.md          ← AI ajanına "buradan başla" talimatı (otomatik eklenir)
└── .kortext/          ← tüm Kortext dosyaları burada (otomatik oluşturulur, git'te)
    ├── agents/
    ├── rules/
    ├── workflows/
    ├── hooks/
    ├── scripts/
    └── workspace/     ← proje hafızası, backlog, context...
```

> ℹ️ `kortext init` ikinci kez çalıştırılırsa mevcut `.kortext/`'e dokunulmaz; workspace içindeki proje verisi korunur.

### 2.3 Başka Bir Makinede Devam Etme

`.kortext/` git'te olduğu için başka bir makinede sadece şunlar yeterli:

```bash
git clone <repo-url>
npm install -g kortext    # bir kez
kortext init              # git hook'larını yeniden kurar
```

### 2.4 Ortam Değişkeni (Opsiyonel ama Önerilen)
...
> Aksi takdirde analiz aşaması başlamaz.
...
```
1. HAZIRLIK
   kortext init → blueprint.md doldur
...
Yeni bir AI oturumu açtığında sistem otomatik olarak:
1. `AGENTS.md`'yi okur ve `+operation-manager` kimliğiyle girer
2. SESSION_BRIEF üretir (aktif context + son handover)
3. Projenin mevcut durumunu tespit eder
4. Hangi komutla devam edileceğini bildirir
...
```
1. kortext init               (projeye kur — bir kez)
2. blueprint.md doldur        (vizyon, hedef kitle, teknoloji)
