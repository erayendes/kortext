...
function buildJudgmentPrompt(project: Project): string {
  return [
    ...
    '',
    'Err towards not-ready when the product itself is unclear. Producing documents from an empty brief',
    'is the failure this gate exists to prevent; asking the human one more question is not a failure.',
    '',
    'But a question earns its place only if the answer would change the product. Details a specialist',
    'settles in their own document — form fields, CMS roles, integrations, consent texts, hosting —',
    'belong to the analysis, not to this gate. For a full brief the usual verdict is ready with',
    'zero questions; do not fill the quota.',
    '',
