...
When a new version is out, the panel says so in a blue strip under the header — it checks when opened and then every hour. **Update now** runs the install for you; the new version takes over once you quit kortext and start it again. If a step is running, the button waits for it rather than swapping the files underneath. By hand, or if the button fails:
