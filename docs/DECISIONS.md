...
> **Numaralandırma notu:** Bölüm 3–4 erken taslakta birleştirildiği için kullanılmadı. Bölüm 15 ile 16 fiziksel sırası append-zamanı kaynaklı (16 önce yazıldı, 15 sonra); ikisi de ~2026-06-06→09. Eski mükerrer "Bölüm 7 (CLI)" → **Bölüm 16** olarak düzeltildi. Bazı eski tasarım kararları (§7.2/7.4/7.5) **superseded** — başlarındaki nota bak.
...
> ⚠️ **Superseded (2026-06-17):** Bu mor/dark-first palet artık geçerli DEĞİL. Güncel sistem **cool-neutral, light-first** (nötr siyah accent) — bkz. [DESIGN.md](./DESIGN.md) §2–§3 ve Bölüm 17. Aşağısı tarihsel kayıttır.
...
> ⚠️ **Superseded:** v4 artık visual spec DEĞİL. Sıra: v4 → v5 IA (Bölüm 11) → **v6 hi-fi** (Bölüm 12) → kanonik [DESIGN.md](./DESIGN.md) (Bölüm 17). `wireframe-v6-hifi.html` tek güncel referans; v2/v3/v4/v5 arşiv.
...
> ⚠️ **Superseded (2026-06-17):** Bu 14-persona hex paleti güncel değil. Artık **16 persona**, kimlik renkleri DESIGN.md §7 roster'ından `--a-*` hue token'larıyla (engineering-manager→red, qa→green vb.); orchestrator=operation-manager (indigo). `persona-colors.ts` tek kaynak olmaya devam eder. Bkz. Bölüm 17.
...
## Bölüm 16 — v3.1 CLI implementation + Faz-3 motor + içerik + approval merdiveni (2026-06-06 → 07)
...
### 16.1 v3.1 CLI = proje-başına port (Eray'ın "A" seçimi, Bölüm 0.1'in çözümü)
...
### 16.2 Bağımlılık üretimi: ajan yazar + motor doğrular (Eray kararı)
...
### 16.3 Motor §5.9 ertelenen dilimler indi
...
### 16.4 Approval merdiveni: staging + preprod (§5.11 implementasyonu)
...
### 16.5 Blocker-clear = otomatik 'blocked' (Eray kararı: dürüst board)
...
### 16.6 CLI sertleştirme + sayfalama
...
### 16.8 Canlı koşu teyidi (gerçek claude ajanı, 2026-06-07)
...
### 16.10 Follow-up implementasyonu (2026-06-07 #2) — gerçek prod merge + tam sayfalama + vocab
...
### 16.11 Onboarding-driven directory + otomatik git (2026-06-08)
...
### 16.12 UAT turu — 4 UAT-güdümlü düzeltme (2026-06-08 #2)
...
### 16.13 Planning enrichment dayanıklılık arkı — UAT #3→#8 (2026-06-08)
...
### 16.14 Multi-model persona routing — main'e merge (2026-06-08, `cbe45b8`)
...
### 16.15 UAT #9 build-fazı — 8 bulgu + GERÇEK-LLM build kanıtı (2026-06-09)
...
### 16.16 UAT #10 — `blocked` ayrı status/sütun OLMAKTAN ÇIKARILDI → türetilen KİLİT bayrağı (2026-06-09)
...
### 16.17 KRİTİK UAT #10 — gate-fail sonsuz bounce döngüsü → 3. fail'de +prime escalation (2026-06-09)
...
### 16.18 UAT #10 — çıplak `kortext start` GUI-first: her zaman sihirbaz + mevcut projeler listesi (2026-06-09)
...
### 16.19 KRİTİK UAT #10h — epic auto-create BASE full-mode ingest'e de uygulandı (tek ortak helper) (2026-06-09)

Temiz UAT (antigravity→codex→claude): ajan 8 task'ı **çıplak `parent_epic: <id>` + `type:epic` container OLMADAN** yazdı, **base full-mode ingest** (`backlog-tanm.1`) üzerinden geçti → her task `FOREIGN KEY constraint failed` → `created 0` → **backlog DB tamamen boş**. #10b'nin epic-auto-create fix'i yalnız `patchBacklogItems`'taydı; full-mode'u kapsamıyordu. Önceki antigravity koşusu tutmuştu çünkü o ajan epic'i `type:epic` item yazmıştı — **saf executor/çıktı varyasyonu yine açığa çıkardı** (aynı sınıf bug'ın 3. yüzü: #5 patch, #10b patch-bare-ref, #10h base-bare-ref).

- **Karar: tek ortak ön-geçiş helper'ı.** `synthesizeMissingEpics(repos, parsed)` çıkarıldı (`backlog-ingest.ts`): batch'te `type:epic` container'ı OLMAYAN + DB'de bulunmayan her çıplak `parent_id` için placeholder epic (id=title) yaratır, `{created, skipped}` döner. **Hem base full-mode `ingestBacklogItems` hem patch-mode `patchBacklogItems` aynı fonksiyonu çağırır** — eski patch-içi "Pre-pass 2" silindi, davranış korundu. Audit action birleşti: `backlog.patch.epic_synthesized` → `backlog.epic_synthesized` (testler action adına bağlı değildi).
- **`batchEpicIds` koruması:** helper, batch'in `type:epic` item olarak tanımladığı id'leri pre-empt ETMEZ (onları normal insert/pre-pass yaratır) — placeholder yalnız gerçekten eksik referanslar için. Böylece gerçek epic başlığı placeholder'la ezilmez.
- **FK-dayanıklı base insert (Eray şartı "backlog asla boş kalmasın"):** full-mode insert loop'unda create'ten ÖNCE `parent_id` çözülürlüğü kontrol edilir; çözülemiyorsa link null'lanır + `backlog.ingest.dangling_parent` uyarısı, item + tüm enrichment (version/owner/model) yine yazılır. patch-mode'un `dangling_parent` deseninin base eşi. Sentez common-path'i kurtarır; bu son-çare garantisi create() istisnası gibi uç durumları kapsar.
- **Sıra (full-mode):** `deriveSyntheticEpics` (bare `epic:` label) → `enforceSymmetricDeps` → `synthesizeMissingEpics` (bare `parent_epic` id) → epics-first insert. Üç farklı "eksik epic" şekli (label / bare-id / in-batch container) de kapanır.
- **Kanıt (deterministik — saf ingest mekaniği, LLM yargısı yok):** uçtan-uca harness gerçek `ingestBacklogFile` full path'i İKİ ajan varyasyonuyla sürdü: (A) çıplak parent_epic + container yok → 3 created, FK skip 0, total 3, her task linked + version/owner/model dolu; (B) epic'i type:epic item yazan → aynı, placeholder değil gerçek epic. **1241 test yeşil** (+5 yeni: synthesizeMissingEpics 3 + base full-mode 2). Gerçek-LLM koşusu gereksiz — fix saf ingest yolunda, gerçek koşu aynı deterministik path'e girer; iki çıktı varyasyonu zaten kapsanıyor.

### 16.20 KRİTİK UAT #10i — no-op implementation tespiti (kod yazmayan dev-cycle başarı sayılmaz) (2026-06-09)

Temiz UAT'ta agy bireysel kotası doldu (429) → implementation adımları codex/claude **fallover**'a düştü → bu koşular dosyaları OKUYUP `exit 0` dönüyor ama **gerçek kod YAZMIYOR** → worktree boş (yalnız scaffold config) ama "succeeded" → item `test`'e geçiyor → gate'ler boş worktree'yi haklı reddediyor → sonsuz bounce churn. 13 item'dan yalnız 1'i (storage.js) gerçek kod üretip merge oldu. Gate'ler doğru çalışıyordu; sorun "başarı"nın tanımındaydı.

- **Karar: "exit 0" yetmez — worktree değişmediyse implementation başarısızdır.** `runItem` success dalına no-op guard eklendi: dev-cycle `succeeded` dönse bile worktree base'e göre byte-aynıysa (kod yazılmadı/commit edilmedi) → recoverable fail. Item `test`'e GEÇMEZ, `in_progress` kalır (driver bir sonraki pass'te tekrar dener), worktree karantinaya alınır, `backlog.implementation.noop` audit yazılır. Bu, #9'daki "boş çıktı = recoverable" sinyal-mantığının **dosya-üretim karşılığı**: "kod değişikliği yok = başarısız".
- **`worktreeHasChanges(worktreePath, baseBranch)` (`worktree.ts`):** iki yönlü diverjans — uncommitted (`git status --porcelain` boş değil) VEYA base'in önünde commit (`git rev-list --count base..HEAD > 0`). **Fail-open:** git cevaplayamazsa (worktree değil / detached / hata) `true` döner — guard yalnız KANITLANMIŞ boş worktree'yi bloklar, gerçek build asla yanlışlıkla atılmaz.
- **Enjeksiyon (test edilebilirlik):** `runItem`'a `worktreeChanged?(lease)` dep'i eklendi (default undefined → guard OFF → eski davranış). `composition` gerçek check'i wire eder: `lease.handle ? worktreeHasChanges(lease.path, lease.handle.baseBranch) : true` (mock lease handle'sız → true). driver `c.worktreeChanged`'i runReadyItems'a geçirir. Böylece run-item unit testleri git-siz kalır; mevcut driver-e2e (kod yazan executor) doğal geçer.
- **#2 (fallover executor kalitesi) — config zaten doğru:** codex executor `--sandbox workspace-write` + `cwd: ctx.worktreePath` kullanıyor (yazma izni + doğru cwd). "Oku ama yazma" davranışsal (prompt/ajan kararı, kota gölgesinde), config bug'ı DEĞİL → no-op guard sebepten bağımsız son-savunma katmanı.
- **#3 (worktree base zamanlaması) — tasarım gereği:** `WorktreeManager.acquire` her worktree'yi `-B branch path development` ile güncel development'tan fork'lar. Aynı drive-pass'inde paralel build edilen item'lar birbirinin merge'ini görmez (run-item phase-1'de acquire, closure phase-3'te merge) — bu doğru izolasyon; sonraki pass güncel development'tan fork'lar. "run-12 storage.js'i görmedi" = paralel-pass izolasyonu, defect değil.
- **Kanıt (deterministik — saf orkestrasyon, LLM yargısı yok):** uçtan-uca harness gerçek `createComposition` + gerçek git worktree'lerle: Pass 1 NoopExecutor (okur, exit 0, yazmaz) → item `in_progress` kalır, `gate_runs` 0 (gate churn yok), `backlog.implementation.noop` audit; Pass 2 WritingExecutor → item `done`. **1246 test yeşil** (+5: worktreeHasChanges 3 + runItem no-op/real 2). Gerçek-LLM gereksiz; fix executor-agnostik orkestrasyon katmanında.

### 16.21 KRİTİK UAT #10j — motor-tarafı yapısal zemin: epic + bağımlılık + versiyon garantisi (executor-bağımsız) (2026-06-09)

codex-primary temiz UAT'ta planning "succeeded" ama backlog yapısız: **18 item, 0 epic, 0 `parent_epic`, 0 `blocked_by`, ~10 versiyon** (BRD "tek versiyon, tek epic" diyordu). owner/version/model 18/18 doluydu ama epic+bağımlılık hiç yok → yapısız board → Eray build'i başlatmadı. Kök: planning enrichment kalitesi **executor-bağımlı** — workflow epic/dep/versiyon üretmeyi ZORUNLU kılıyor ama codex talimatları yok saydı (antigravity koşularında tutuyordu). #10h epic auto-create yalnız ajan bir `parent_epic` referansı VERİRSE devreye giriyor; codex hiç vermedi → sentezlenecek epic yok → epic 0. **Saf executor varyasyonu yine açığa çıkardı.**

- **Karar: motor-tarafı GARANTİ (talimat değil).** Yeni `ensureBacklogStructure(repos, opts)` — planning bittiğinde çalışan, executor-bağımsız, idempotent + muhafazakâr yapısal zemin. Üç boyut, her biri yalnız KOMPLE eksik/patolojik olduğunda ateşler (sağlam antigravity çıktısı dokunulmaz):
  1. **Epic garantisi:** `epics.length === 0 && nonEpics.length > 0` → 1 varsayılan epic (`${code}-E01`, başlık=proje adı) yaratır + tüm köksüz task'ları reparent eder. "Board asla epic'siz kalmasın" (Eray şartı).
  2. **Bağımlılık türetme:** hiç `blocked_by` yoksa + ≥2 task → id-sırası (`localeCompare numeric`) lineer zincir (`T-2.blocked_by=[T-1]`, `T-3=[T-2]`…). Amaç "18 paralel-aynı-tabandan olmasın" (UAT #9 merge-conflict stall'ı). Aşırı serialize eder ama bozuk çıktı için güvenli fallback; sağlam çıktı kendi deps'ini verince hiç ateşlemez.
  3. **Versiyon aklı:** `distinctVersions > nonEpics/2` (≈<2 item/versiyon — patolojik) → en erken versiyona toplar. BRD-parse YOK; saf sayısal sezgi (18 item/10 versiyon → topla; 18 item/3 versiyon → bırak).
- **Reparent/reversion:** `updatePlanningFields`'i mevcut alanları koruyarak tek-alan değiştirme (`rewritePlanning` yardımcısı); blocked_by frontmatter'da → `updateFrontmatter`. `compareVersionLabels` numeric-segment (v0.2<v0.10).
- **Wiring (timing = board görünürlüğü için kritik):** `index.ts` `triggerAnalysis` `.then`'inde, chain `planning-pipeline`'a ulaşıp `status==='succeeded'` olunca çağrılır → sonra `writeBacklogYamlFromDb` ile `backlog.yaml` yeniden serialize (Board + sonraki tüketici normalize yapıyı görür). Best-effort try/catch (yapısal floor hatası planning sonucunu bozmaz). Build-time değil planning-completion'da, çünkü kullanıcı Board'u görüp build'i başlatacak.
- **Kanıt (deterministik — saf ingest-sonrası mekanik, LLM yargısı yok):** harness codex-bozuk durumu birebir kurar (18 task/0 epic/0 dep/10 versiyon) → floor → 1 epic (hepsi parented) + 17 zincir + versiyon 10→1 + `selectBuildableItems` **18 paralel yerine 1 head** (NOT-001). **1253 test yeşil** (+7: ensureBacklogStructure epic/dep/version/idempotent). Motor garantisi executor-bağımsız → gerçek-codex koşusu fix'i değiştirmez (aynı DB durumuna girer).
- **Kapsam dışı (TODO'da):** (A2) `planning-pipeline.md` talimat sertleştirme (ikincil — floor güvence); **(B) build-start** — planning sonrası development-cycle otomatik tetiklenmiyor / kullanıcının net başlatma yolu yok (ayrı bulgu). Bu karar planning'in YAPISINI garanti eder, build'i BAŞLATMAZ.

---

## Bölüm 17 — v6 dashboard migrasyonu + UAT #10k/L/M + tasarım-sistemi hizalaması (2026-06-09 → 17)

> Bölüm 15/16'dan sonraki ~8 günün kararları. Çoğu tek release commit'inde toplandı: **`a6c0ce6` feat(ui): v6 dashboard migration + DESIGN.md spec alignment** (59 dosya, 1313 test yeşil, `origin/main`'e push).

### 17.1 UAT #10k — epic floor ERKEN (ingest'te, planning-completion'da değil)
#10j'nin `ensureBacklogStructure`'ı yalnız planning-completion hook'unda çalışıyordu → planning boyunca Board epic 0, erken durdurulursa epic hiç yok. **Karar:** epic-garanti `ensureEpicFloor` olarak ayrıldı (`backlog-ingest.ts`) ve **`ingestBacklogFile` sonunda** çalışır — backlog DB'ye düştüğü AN ≥1 epic + köksüz task'lar bağlı. `ingestBacklogPatchFile` her patch sonrası idempotent korur (epic varsa no-op). Completion-hook çağrısı son savunma olarak duruyor.

### 17.2 UAT #10L — codex implementation KOD YAZMIYOR + no-op tespiti sıkılaştırma
codex-primary'de implementation adımları dosya okuyup `exit 0` dönüyor, kod yazmıyordu (antigravity yazıyordu → executor-bağımlı). **Kararlar:** (a) **kök = prompt** — hiçbir executor'a HANGİ item'ın uygulanacağı söylenmiyordu; `ExecutorContext.itemContext` (id/başlık/açıklama/AC) her adımın prompt'una taşınır; codex/gemini prompt'una CWD + `--- Mandate ---` (dosya ÜRET; salt-okuma=fail) eklendi (sandbox/cwd zaten doğruydu). (b) **no-op sıkılaştırma** — `worktreeHasMeaningfulChanges`: değişen dosyalar arası en az 1 app/kod uzantısı (.html/.css/.js/.ts/…) yoksa recoverable-fail (config-only başarı sayılmaz). (c) **version-floor** — hiçbir item'da version yoksa hepsine v0.1 atanır. Canlı kanıt: codex 0.139.0 58sn'de index.html+css+js+test yazdı.

### 17.3 UAT #10M — üretilen kod MERGE'e girmiyordu → commit-before-merge
Codex kod yazdı, gate'ler commit'siz worktree'yi okuyup geçti AMA merge worktree'nin **commit'li** halini alıyordu → boş merge → sahte "done", `development` boş. **Karar: ajanın commit'ine güvenme, MOTOR commit etsin.** `commitWorktreeChanges` (`worktree.ts`): `git add -A` + commit, inline identity + `--no-verify` + `gpgsign=false`, best-effort. `runItem` başarı dalında no-op guard'dan ÖNCE `commitWorktree(lease)` çağırır. No-op guard `worktreeHasMeaningfulCommit`'e (yalnız `base...HEAD` commit'li tarihçe) bağlandı → commit'li app dosyası yoksa bounce. Üç görünüm (ajan-yazımı/gate-okuması/merge-taşıması) tek commit noktasında eşitlenir. Regresyon: `WritesButNeverCommitsExecutor` e2e.

### 17.4 v6 dashboard React migrasyonu — gerçek projeye bağlı + status-driven gate
Bölüm 11/12'nin v5 IA + v6 hi-fi wireframe kararları gerçek React'e indi: tüm ekranlar (Dashboard · Board · Memory · Foundation · References · Reports · Project info · Integrations · Environments) gerçek `/api/*` verisine bağlandı (wireframe'deki baked diziler kaldırıldı). **Onboarding gate `main.tsx`'te status-driven** — `blueprint.status === 'approved'` ise app, değilse onboarding sihirbazı. Footer `active/idle/blocked` olarak etiketlendi; ajan ikonu nötr (statü rengiyle boyanmaz).

### 17.5 Yeni ekran/backend dilimleri (v6 kapsamı)
- **Environments — per-env:** değişkenler `dev/staging/production` ayrı dosyalarda (`.kortext/env/<env>.env`) + "All" matris; **public** (düz değer) / **secret** (maskeli) ayrımı. Domain/app-id gibi kimlik bilgileri Environments'a ait (onboarding'e değil) kararı.
- **LLM models — global engine ekranı:** Agent models + LLM Auth tek "LLM models" ekranında (per-LLM auth method + elle girilen modeller + API key); Agents ekranına model ataması. Global `~/.kortext/` config.
- **Integrations — yeniden yapı (store-only):** GitHub hero (repo/branch/auto-commit/PR-approval config) üstte; Vercel·Supabase·Firebase + Sentry·Stripe gruplu kartlar. Token'lar maskeli saklanır, **dışarı çağrı yok** (operasyonel bağlama ayrı epic — TODO).
- **Onboarding** Initialize (sidebar shell + BRD paste/upload + executor drag-reorder) + Setup ("Project initializing…" faz rayı + activity stream; veri şimdilik temsilî).
- **Foundation route**, **project-danger** (reset/remove/delete) + **orphan-sweep** (`cmd-orphans.ts`: kayıtsız kortext-port daemon'larını bul+SIGTERM) CLI/route'ları.

### 17.6 Tasarım-sistemi hizalaması — DESIGN.md kanonik oldu (2026-06-17)
Eski `development/DESIGN.md` (mor, dark-first) yerini **yeni otoritatif spec**e bıraktı (cool-neutral, light-first, themeable; §0–§8 token+bileşen+ikon+vocabulary). Kod buna hizalandı:
- **Butonlar** → kanonik set (`primary/secondary/ghost/danger/success/sm/icon`); legacy `btn-pri`/`btn-line`/`btn-approve`/mükerrer `.btn` tabanı silindi (dark-mode `#fff` bug'ı çözüldü).
- **İkonlar** → §6 tam eşleme app geneli (folder nav ikonları, item-type/detay glyph'leri, `github→GitMerge`, `sentry→Radio`, `cli→SquareTerminal`, `lock→LockKeyhole`, `debt→Coins`, `epic→Bookmark`, dosya satırları→FileText, ask-ai→BotMessageSquare, child→ListTree…).
- **Toggle off-state** kararı: light=koyu knob+beyaz halka / dark=beyaz knob+koyu halka (Eray tercihi; DESIGN.md §5.5 buna göre güncellendi — spec'in ilk yazımının tersi).
- **Density** varsayılanı `comfortable` (--d-scale 1.2); compact opt-in.
- **Persona kimlik renkleri** → §7 roster'ı `--a-*` hue token'larıyla (eski v4 hex'leri gitti); 16 persona; `+orchestrator` = operation-manager (indigo) alias'ı.

### 17.7 Release + repo temizliği (2026-06-17)
Tüm v6 + hizalama işi tek commit'te (`a6c0ce6`) `main`'e fast-forward + **`origin/main`'e push** (Eray açık onayı; 1313 test yeşil). 8 atıl/merge'li local branch silindi (claude/* + feat/mcp-headless + worktree-feature). TODO.md sadeleştirildi (279→148 satır, bitmiş post-mortem'ler buraya taşındı). Doküman temizliği (eski wireframe/PERSONA-ICONS/specs arşivi) TODO'da.
