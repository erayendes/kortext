...
Üçüncüsü, **effort**, bu kavramı bilen CLI'larda görünür — `claude --effort`, codex'in `model_reasoning_effort`'u, `agy --effort` — o CLI'ın kendi seçicisinin sunduğu seviyelerle. Model gibi, ayarladıktan sonra başlayan koşulara ulaşır. Model gibi, ayarladıktan sonra başlayan koşulara ulaşır; motoru değiştirdiğinizde yeni CLI'ın almadığı seviye düşer.
