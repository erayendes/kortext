...
**The update strip**, at the top, in blue, only when npm has a newer version — the panel looks once when opened and then every hour, so a release lands while it sits open. **Update now** installs it; then quit kortext and start it again.
